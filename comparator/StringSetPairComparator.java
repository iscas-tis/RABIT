...
public class StringSetPairComparator  implements Comparator<Pair<String,SortedSet<FAState>>> {
	public int compare(Pair<String,SortedSet<FAState>> s1, Pair<String,SortedSet<FAState>> s2){
		if(s1.getLeft().compareTo(s2.getLeft())!=0){
			return s1.getLeft().compareTo(s2.getLeft());
		}else{
			return compare(s1.getRight(),s2.getRight());
		}
	}
